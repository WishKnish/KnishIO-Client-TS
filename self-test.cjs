...
/**
 * Knish.IO TypeScript SDK Self-Test Script (JavaScript version)
 * 
 ...
 * 
 * This JS version works around build issues with the TypeScript SDK.
 */

const fs = require('fs');
const path = require('path');

// Try to import from the built CJS version
let SDK;
try {
  SDK = require('./dist/index.cjs');
} catch (error) {
  console.error('Failed to load SDK from dist/index.cjs:', error.message);
...
// Embedded test configuration for SDK self-containment
...
// Support optional external config override
...
// crossSdkCompatible starts FALSE. It was initialised to `true`, making "fully cross-SDK
// compatible" the default state before a single peer molecule had been examined — so every
// early return out of testCrossSdkValidation published a pass. A verdict must be earned;
// the safe default for a check that has not run is "failed".
//
// crossValidation records the COVERAGE behind the verdict, which the boolean alone cannot
// convey: it cannot distinguish "validated seven peers" from "validated nothing".
const results = {
  sdk: 'TypeScript',
...
async function testCrypto() {
  ...
  try {
    ...
    log(`  Generated secret length: ${secret.length}`, 'yellow');
    log(`  First 64 chars: ${secret.substring(0, 64)}...`, 'yellow');
    log(`  Expected length: ${testConfig.secret.length}`, 'yellow');
    log(`  Expected first 64: ${testConfig.secret.substring(0, 64)}...`, 'yellow');
    ...
    log(`  Generated bundle: ${bundle}`, 'yellow');
    log(`  Expected bundle: ${testConfig.bundle}`, 'yellow');
...
async function testMetaCreation() {
  ...
  try {
    ...
    const secret = generateSecret(testConfig.seed);
    const bundle = 'fee9c2b9a964d060eb4645c4001db805c3c4b0cc9bba12841036eba4bf44b831'; // Fixed bundle for deterministic testing
    ...
    // Fix bundle for deterministic testing (override TypeScript bundle generation)
    sourceWallet.bundle = 'fee9c2b9a964d060eb4645c4001db805c3c4b0cc9bba12841036eba4bf44b831';
...
async function testSimpleTransfer() {
  ...
  try {
    ...
    const sourceBundle = 'fee9c2b9a964d060eb4645c4001db805c3c4b0cc9bba12841036eba4bf44b831'; // Fixed bundle for deterministic testing
 ...
 * gauntlet at all prior to this. For each buffer_deposit_conservation /
 * buffer_withdraw_conservation vector we build + sign the molecule and assert:
 * atom shape/values match the vector, the V+B sum == expectedSum (full-balance
 * debit conserves even for a PARTIAL op), AND molecule.check() accepts the
 * molecule (the isotopeV/isotopeB cross-isotope bypass — V-only atoms would
 * not sum to zero on their own). Molecular hashes are NOT frozen (random
 * positions).
 *
  ...
  try {
    ...
    for (const tv of v.buffer_deposit_conservation.tests) {
      ...
      try {
        ...
        if (!checkOk) checkError = 'Validation returned false (no exception thrown)';
      } catch (error) {
        checkOk = false;
      ...
      logTest(`deposit ${tv.name} conserves (V+B sum 0; cross-isotope bypass)`, ok, checkError);
      allPass = allPass && ok;
    ...
    for (const tv of v.buffer_withdraw_conservation.tests) {
      ...
      const ok = shape && sum.toString() === tv.expectedSum && checkOk;
      logTest(`withdraw ${tv.name} conserves (B+V sum 0; cross-isotope bypass)`, ok, checkError);
      allPass = allPass && ok;
      lastHash = molecule.molecularHash;
      atomTotal += molecule.atoms.length;
    }
...
async function testCrossSdkValidation() {
    ...
    for (const [moleculeType, moleculeData] of Object.entries(published)) {
      try {
        if (moleculeType === 'mlkem768') {
          ...
          const moleculeJson = JSON.parse(moleculeData);
          
          // Create a Molecule instance with minimal constructor args
          const molecule = new Molecule({ 
            secret: null,
            bundle: moleculeJson.bundle || null,
            sourceWallet: null,
            cellSlug: moleculeJson.cellSlug || null
          });
          
          // Populate all fields from JSON
          Object.keys(moleculeJson).forEach(key => {
            if (key === 'atoms') {
              // Ensure atoms are proper Atom instances
              molecule.atoms = moleculeJson.atoms.map(atomData => {
                const atom = new Atom({
                ...
                // Set additional atom properties
                atom.index = atomData.index;
                atom.otsFragment = atomData.otsFragment;
                atom.createdAt = atomData.createdAt;
                atom.batchId = atomData.batchId || null;
                atom.meta = atomData.meta || [];
                return atom;
              });
            } else if (key !== 'secret' && key !== 'sourceWallet' && key !== 'remainderWallet') {
              // Copy other properties except transient ones
              molecule[key] = moleculeJson[key];
            }
          });
        
        // Create source wallet if available for proper validation
        let sourceWallet = null;
        if (moleculeJson.sourceWallet) {
          // For value transfers, we need a source wallet with balance for validation
          sourceWallet = new Wallet({
            secret: null, // Not needed for validation
            token: moleculeJson.sourceWallet.token,
            position: moleculeJson.sourceWallet.position
          });
          // Set the balance for validation
          sourceWallet.balance = moleculeJson.sourceWallet.balance || 0;
          sourceWallet.address = moleculeJson.sourceWallet.address;
        }
        
        // Now use the molecule's check() method for full validation
        let isValid = false;
        try {
  ...
  // so it records "nothing went wrong", not "everything was checked". Require both.
...
async function main() {
  try {
    ...
    if (process.env.KNISHIO_CROSS_VALIDATION_ONLY === 'true') {
      ...
      const existingResultsPath = path.join(sharedResultsDir, 'typescript-results.json');
    ...
    const tokenCreationPassed = await testTokenCreation();
    const walletCreationPassed = await testWalletCreation();
    const shadowWalletClaimPassed = await testShadowWalletClaim();
    const bufferFamilyPassed = await testBufferFamily();
    const mlkemPassed = await testMLKEM768();
    const negativePassed = await testNegativeCases();
    const crossSdkPassed = await testCrossSdkValidation();
    
    // Overall pass/fail is derived generically from results.tests rather than a
    // hand-tallied boolean chain — a hardcoded list silently drops any test whose
    // variable isn't threaded through (this file's own tokenCreation/walletCreation/
    // shadowWalletClaim results were already missing from the old tally, so a real
    // regression in any of them would never have flipped the exit code). A skipped
    // test (fixture absent, KNISHIO_REQUIRE_VECTORS unset) counts as neither a pass
    // nor a failure.
    const allPassed = Object.values(results.tests).every(t => t.passed || t.skipped) && crossSdkPassed;
