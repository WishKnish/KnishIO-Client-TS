...
/**
 * DataBraid Embedding Status — E2E Test (TypeScript SDK)
 *
 * Tests the TS SDK's embedding status query against a live validator:
...
import { KnishIOClient, generateSecret } from '../../dist/index.js'
...
async function main () {
  console.log(`\n${B}${C}DataBraid Embedding Status — E2E Test (TypeScript SDK)${X}`)
  ...
  if (!supportsEmbedding) {
    ...
    try {
      ...
      if (response) {
        ...
        if (payload && Array.isArray(payload) && payload.length === 3) {
          ...
          fail('Bulk query — unexpected payload', `length: ${payload?.length}`)
        }
      } else {
    ...
    // 3c: Write meta, then check embedding status
    ...
    try {
      ...
      const testMetaType = 'EmbedTestTS'
      const testMetaId = `e2e_ts_${Date.now()}`

      const metaResponse = await client.createMeta({
        metaType: testMetaType,
        metaId: testMetaId,
        meta: {
          description: 'TS SDK E2E test for embedding status observability',
