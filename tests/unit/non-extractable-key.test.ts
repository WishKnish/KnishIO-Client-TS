...
import NonExtractableKeySecretStorageProvider, {
  MemoryKeyStore
} from '../../src/storage/NonExtractableKeySecretStorageProvider'
...
describe('NonExtractableKeySecretStorageProvider', () => {
  it('round-trips secrets using MemoryKeyStore', async () => {
  ...
  it('allows second instance sharing both stores to retrieve', async () => {
    const backend = new MemoryStorageBackend()
    const keyStore = new MemoryKeyStore()
  ...
  it('throws unavailable if record exists but key is missing from keyStore', async () => {
    const backend = new MemoryStorageBackend()
    const keyStore = new MemoryKeyStore()
  ...
  it('throws decryptionFailed if key in keyStore is wrong', async () => {
    const backend = new MemoryStorageBackend()
    const keyStore = new MemoryKeyStore()
  ...
  it('requires recoveryPassphrase unless allowUnrecoverable is true', async () => {
    const backend = new MemoryStorageBackend()
    const keyStore = new MemoryKeyStore()
  ...
  it('stores recovery envelope and recovers successfully', async () => {
    const backend = new MemoryStorageBackend()
    const keyStore = new MemoryKeyStore()
    const provider = new NonExtractableKeySecretStorageProvider({
      backend,
