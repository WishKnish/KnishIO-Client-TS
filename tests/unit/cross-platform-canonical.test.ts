 ...
 * one skipped test naming the absent path (mirrors generate-secret-parity.test.ts).
 */
...
type CrossPlatformVectors = {
  vectors: {
    ...
    secret_storage_envelope: {
      tests: Array<{
        storageKey?: string
        bundleHash?: string
        payload: { metadata: { hardwareBacked: boolean } }
...
const FIXTURE = resolve(__dirname, '../../../shared-test-results/cross-platform-test-vectors.json')
const fixture: CrossPlatformVectors | null = existsSync(FIXTURE) ? JSON.parse(readFileSync(FIXTURE, 'utf8')) : null

if (!fixture) {
  it.skip('cross-platform-canonical.test.ts: skipped — ../shared-test-results/cross-platform-test-vectors.json not found (standalone checkout)', () => {})
} else {
  ...
  describe('Secret storage envelope parity with master vector', () => {
    it('master vector payload matches the frozen envelope constant', () => {
      const masterPayload = vectors.secret_storage_envelope.tests[0].payload
    ...
    it('master vector tests[1] is the frozen recovery record', () => {
      const t = vectors.secret_storage_envelope.tests[1]
