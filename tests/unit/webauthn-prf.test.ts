...
describe('WebAuthnPrfSecretStorageProvider', () => {
  ...
  it('unenrolls and allows re-enrollment', async () => {
    const backend = new MemoryStorageBackend()
    const credentials = createFakeCredentials()
    const provider = new WebAuthnPrfSecretStorageProvider({
      backend,
  ...
  it('wraps NotAllowedError into SecretStorageException.unavailable', async () => {
    const backend = new MemoryStorageBackend()
    ...
    // Pre-populate an enrolled record so unlock attempts credentials.get
