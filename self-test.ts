...
function setFixedTimestamps(molecule: any): void {
...
function createFixedRemainderWallet(secret: string, token: string): any {
  return new Wallet({
    secret: secret,
    token: token,
...
function logTest(testName: string, passed: boolean, errorDetail?: string | null): void {
 ...
 */
function inspectMolecule(molecule: Molecule, name: string = 'molecule'): void {
  ...
  molecule.atoms.forEach((atom, index) => {
    const value = parseFloat(atom.value as string) || 0;
    totalValue += value;
 ...
 */
function diagnoseValidation(molecule: Molecule, senderWallet: Wallet | null, name: string = 'molecule'): void {
  ...
  try {
    ...
    try {
      ...
      log(`  Basic validation error: ${(validationError as Error).message}`, 'red');
    }
    
  } catch (error) {
    log(`  ❌ Diagnostic error: ${(error as Error).message}`, 'red');
  }
}
 ...
 */
async function testCrypto(): Promise<boolean> {
  log('\n1. Crypto Test', 'blue');
 ...
 */
async function testMetaCreation(): Promise<boolean> {
 ...
 */
async function testSimpleTransfer(): Promise<boolean> {
 ...
 */
async function testComplexTransfer(): Promise<boolean> {
 ...
 */
async function testMLKEM768(): Promise<boolean> {
 ...
 */
async function testNegativeCases(): Promise<boolean> {
 ...
 */
async function testCrossSdkValidation(): Promise<boolean> {
    ...
    for (const [moleculeType, moleculeData] of Object.entries(otherResults.molecules || {})) {
      try {
        if (moleculeType === 'mlkem768') {
          ...
          const mlkemData = JSON.parse(moleculeData as string);
          ...
          try {
            ...
            log(`    Failed to encrypt for ${sdkName}: ${(error as Error).message}`, 'red');
            mlkemValid = false;
          }
          ...
          const molecule = Molecule.fromJSON(moleculeData as string, {
          ...
          let sourceWallet: Wallet | null = molecule.sourceWallet;
          ...
          try {
            ...
            log(`    Validation error: ${(error as Error).message}`, 'red');
            isValid = false;
          }
    ...
    if (process.env.KNISHIO_CROSS_VALIDATION_ONLY === 'true') {
      ...
      if (fs.existsSync(existingResultsPath)) {
        try {
          ...
          log(`⚠️  Could not load existing results: ${(error as Error).message}`, 'yellow');
        }
      }
